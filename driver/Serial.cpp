...
void SERCOM4_Handler(){
	...
	if(c == '\n') {
		...
		memcpy(serial.line_buffer, recv_buffer, recv_buffer_len+1);
		serial.line_received_cb(serial.line_buffer); // copy buffer to a string and call dedicated function
		recv_buffer_len = 0;
	} else {
		if (recv_buffer_len < 63) recv_buffer[recv_buffer_len++] = c;
		if (recv_buffer_len >= 63){
			memcpy(serial.line_buffer, recv_buffer, recv_buffer_len+1);
			serial.receive_buffer_full_cb(serial.line_buffer);
			recv_buffer_len = 0;
		}	
	}
}
...
//	USART Config:
	...
	uart.BAUD.bit.BAUD = 63019; // 63019->115200baud, Table 31-2 Asynchronous Arithmetic
